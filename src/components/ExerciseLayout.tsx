...
import type { Exercise } from '../items/types'
...
export function ExerciseLayout({ exercise, children }: ExerciseLayoutProps) {
  return (
    <article>
      <header className="mb-6 flex flex-col border-b border-gray-200 dark:border-gray-800">
        <div className="flex flex-col gap-2">
          <div className="flex items-center gap-2">
            <p>Level: </p>
            <p
              className={`flex w-fit rounded-xl border-2 px-2 py-1 font-bold capitalize ${levelBadgeClass(exercise.level)}`}
            >
              {exercise.level}
            </p>
          </div>
          <div className="mb-5 rounded-lg border border-blue-200 border-l-4 border-l-blue-600 bg-blue-50 px-5 py-4 dark:border-blue-900 dark:border-l-blue-500 dark:bg-blue-950/40">
            ...
            <p className="text-lg font-medium leading-snug text-gray-900 dark:text-gray-100">{exercise.question}</p>
          </div>
        </div>
        <h2 className="mb-2 text-2xl font-semibold text-gray-900 dark:text-gray-100">{exercise.title}</h2>
        <p className="mb-3 text-gray-600 dark:text-gray-300">{exercise.description}</p>
